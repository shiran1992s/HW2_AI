...
class Player(AbstractPlayer):
    ...
    # DONE: add here the utility, succ, and perform_move functions used in AlphaBeta algorithm
    ...
    if blocked_cells / (board_size - total_free_cells) < 0.4:
        ...
        if state.fruit_life_time > 0:
            value = ((3 * player_moves_number) - (1 * rival_moves_number)
                     + (len(blocking_moves)) + (2 * len(successor_available_moves)) + 0.2 * state.points)
        else:
            value = (3 * player_moves_number) - (1 * rival_moves_number) \
